...
def run(init_lr=0.1,
    ...
    for data in dataloaders["test"]:
        ...
        labels = np.argmax(labels[:, :, 0]).unsqueeze(0)

        # per_frame_logits = i3d(inputs)
        logits, per_frame_logits = i3d(inputs)
    ...
    logging.info('Seed:{}. top-k average per class acc: {}, {}, {}'.format(seed, top1_per_class, top5_per_class, top10_per_class))
...
if __name__ == '__main__':
    ...
    init_logging(os.path.join('log/test_depth6_topk_{:s}_{}_log.txt'.format("0818", num_classes)))

    
    
    run(mode=mode, root=root, save_model=save_model, train_split=train_split, weights=weights, seed=28)
